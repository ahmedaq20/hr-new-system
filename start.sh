...
if [ "$1" = "--build" ] || [ "$1" = "-b" ]; then
    ...
    echo "Starting Docker containers (use --build to rebuild)..."
    if ! docker images | grep -q "hrms-backend\|hrms-frontend"; then
        echo "Images don't exist, building for first time..."
        docker-compose -f docker/docker-compose.yml up -d --build
    else
...
docker-compose -f docker/docker-compose.yml exec -T frontend-builder sh -c "cp -r /app/dist/* /output/ 2>/dev/null || true"
