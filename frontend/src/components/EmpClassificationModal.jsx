...
function EmpClassificationModal({ show, onClose, onSave, item = null, isMutating }) {
        ...
        >
            ...
            >
                <Modal.Title className="fs-5 fw-bold text-white">
                    {isEdit ? "تعديل تصنيف الموظب" : "إضافة تصنيف جديد"}
                </Modal.Title>
            </Modal.Header>
            <Modal.Body className="p-4 bg-light">
                <Form className="text-end">
                    <Form.Group className="mb-0">
                        <Form.Label className="fw-bold text-secondary small">اسم التصنيف</Form.Label>
                        <Form.Control
                            type="text"
                            placeholder="مثال: هندسة، إحصاء، إدارة أعمال..."
            ...
            <Modal.Footer className="border-0 pb-4 px-4 bg-light rounded-bottom-4">
                ...
                <Button
                    ...
                    disabled={isMutating}
                >
                    {isMutating ? (isEdit ? 'جاري الحفظ...' : 'جاري الإضافة...') : (isEdit ? "حفظ التغييرات" : "إضافة التصنيف")}
                </Button>
            </Modal.Footer>
        </Modal>
    );
...
export default EmpClassificationModal;
