...
function AcademicCertificatesModal({ show, onClose, onSave, item = null, isMutating }) {
    ...
    const { items: certificates } = useReferenceData('CERTIFICATE');
    ...
    useEffect(() => {
        if (item) {
            setForm({
                ...
                reference_value_id: item.reference_value_id || "",
    ...
    const handleSave = () => {
        if (!isEdit && (!form.employee_id || !form.reference_value_id || !form.file)) return;
        onSave(form);
    };
        ...
        <Modal show={show} onHide={onClose} centered className="modal-modern" size="lg">
            <Modal.Header closeButton closeVariant="white" className="border-0 shadow-none d-flex justify-content-between align-items-center" style={{ background: '#002F6C' }}>
                <Modal.Title className="fs-5 fw-bold text-white m-0">
                    {isEdit ? "تعديل حالة الشهادة" : "إضافة شهادة أكاديمية جديدة"}
                </Modal.Title>
            </Modal.Header>
                ...
                <Form className="mt-3">
                    {!isEdit && (
                        <div className="row">
                            ...
                            <div className="col-md-6 text-end">
                                <CSelect
                                    label={<span>الشهادة <span className="text-danger">*</span></span>}
                                    name="reference_value_id"
                                    options={certificateOptions}
                                    value={certificateOptions.find(opt => opt.value === form.reference_value_id)}
                                    onChange={handleSelectChange}
                        ...
                        <Form.Group className="mb-3 text-end">
                            ...
                            {isMutating && !isEdit && (
                                <div className="mt-3">
                                    ...
                                    <small className="text-primary d-block mt-1 fw-bold text-center animate-pulse">جاري رفع الملف... يرجى الانتظار</small>
                                </div>
                            )}
                        </Form.Group>
                    )}
            ...
            <Modal.Footer className="border-0 pt-0 pb-4 px-4">
                <div className="d-flex gap-2 w-100">
                    ...
                    <Button
                        ...
                        disabled={isMutating}
                    >
                        {isMutating ? "جاري الحفظ..." : (isEdit ? "حفظ التغييرات" : "حفظ الشهادة")}
                    </Button>
                </div>
            </Modal.Footer>
        </Modal>
...
export default AcademicCertificatesModal;
