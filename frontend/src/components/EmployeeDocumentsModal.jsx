...
function EmployeeDocumentsModal({ show, onClose, onSave, item = null, isMutating }) {
        ...
        >
            ...
            >
                <Modal.Title className="fs-5 fw-bold text-white">
                    {isEdit ? "تعديل نوع المستند" : "إضافة نوع مستند جديد"}
                </Modal.Title>
            </Modal.Header>
            <Modal.Body className="p-4 bg-light text-end">
                <Form>
                    <Form.Group className="mb-0">
                        <Form.Label className="fw-bold text-secondary small">اسم المستند</Form.Label>
                        <Form.Control
                            type="text"
            ...
            <Modal.Footer className="border-0 pb-4 px-4 bg-light rounded-bottom-4">
                ...
                <Button
                    ...
                    disabled={isMutating}
                >
                    {isMutating ? (isEdit ? 'جاري الحفظ...' : 'جاري الإضافة...') : (isEdit ? "حفظ التغييرات" : "إضافة المستند")}
                </Button>
            </Modal.Footer>
        </Modal>
    );
