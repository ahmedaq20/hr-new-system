...
import { useUpdateCourseRequest } from "../hooks/useAddCourseRequest";

interface EditCourseRequestProps {
...
const EditCourseRequest: React.FC<EditCourseRequestProps> = ({ participant, onSuccess, onCancel }) => {
    ...
    const updateRequestMutation = useUpdateCourseRequest();
    ...
    useEffect(() => {
        if (participant) {
            ...
            setCourseName(participant.course_name || "");
            setInstitution(participant.provider || "");
            setHours(participant.hours?.toString() || "");

            // Parse date string MM/YYYY to Date object
            if (participant.date && typeof participant.date === 'string' && participant.date.includes('/')) {
                const [month, year] = participant.date.split('/');
                const date = new Date(parseInt(year), parseInt(month) - 1, 1);
                setCourseDate(date);
            } else if (participant.date) {
                // Fallback for other formats
                setCourseDate(new Date(participant.date));
            } else {
                setCourseDate(null);
            }
        ...
        >
            <h5 className="mb-4 text-center d-flex align-items-center justify-content-center gap-2" style={{ fontWeight: 600, color: "#016A74" }}>
                ...
                {participant.approval_status === 'pending' ? (
                    ...
                    <Badge bg="secondary-subtle" className="text-secondary border border-secondary-subtle px-2 py-1 fw-normal" style={{ fontSize: '0.8rem' }}>{participant.approval_status}</Badge>
                )}
            </h5>
            ...
            <Form onSubmit={handleSubmit}>
                ...
                <Form.Group className="mb-4">
                    <Form.Label className="fw-bold">تحديث الشهادة (اختياري)</Form.Label>
                    <Form.Control
                        type="file"
                    ...
                    {participant?.certificate_url && (
                        <div className="mt-2">
                            <a href={participant.certificate_url} target="_blank" rel="noreferrer" className="btn btn-sm btn-outline-info" style={{ borderRadius: "8px" }}>
                                عرض المرفق الحالي
                            </a>
                        </div>
                    )}
                ...
                <div className="d-flex justify-content-end gap-2 mt-4">
                    ...
                    >
                        {updateRequestMutation.isPending ? <Spinner animation="border" size="sm" /> : "حفظ التعديلات"}
                    </Button>
                </div>
            </Form>
        </Card>
